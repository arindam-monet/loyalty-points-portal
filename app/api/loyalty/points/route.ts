...
const COMPANY_ID = 'company1'; // Specific to this loyalty program
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const companyDoc = await db.collection(COLLECTION_NAME).findOne({ companyId: COMPANY_ID });
    if (!companyDoc) {
      return NextResponse.json(
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const currentPoints = customer.points.reduce((total: number, entry: any) => {
    ...
    return NextResponse.json({ message: 'Loyalty points updated successfully.' });
  } catch (err) {
